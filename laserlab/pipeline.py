...
def run_experiment(experiment_dir: Path, profile_name: str = "baseline", blind_seed: int = 1) -> dict[str, Any]:
    try:
        import cv2
    ...
    samples = _build_samples(experiment_dir, run_dir, manifest)
    ...
    aggregate = summarize_results(results, seed=blind_seed)
    run_record = {
        "schema_version": 1,
        "run_id": run_id,
...
def _build_samples(experiment_dir: Path, run_dir: Path, manifest: dict[str, Any]) -> list[dict[str, Any]]:
    import cv2
    ...
    for capture in manifest.get("captures", []):
        for frame in capture.get("frames", []):
            ...
            if capture["label"] == "laser":
                ...
                generated_controls.extend(generate_matched_controls(experiment_dir, run_dir, capture, frame, image))
